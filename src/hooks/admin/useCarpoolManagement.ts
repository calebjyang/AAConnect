...
export function useCarpoolManagement() {
  ...
  const fetchSignups = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      const signupsData = await getCollection('rides');
      setSignups(signupsData as RideSignupAdmin[]);
      // Set default week to most recent by quarter/week order
      const weeks = Array.from(new Set(signupsData.map((s: any) => s.aftereventWeek).filter(Boolean)));
      if (weeks.length > 0) {
  ...
  const loadAssignments = useCallback(async () => {
    ...
    try {
      const assignmentsData = await getCollection('carpoolAssignments');
      
      // Find the document for the selected week
      const assignmentDoc = assignmentsData.find((doc: any) => doc.week === selectedWeek);
      
      if (assignmentDoc) {
        const data = assignmentDoc as CarpoolAssignmentDoc;
  ...
  const saveAssignments = useCallback(async () => {
    ...
    try {
      ...
      await setDoc(`carpoolAssignments/${selectedWeek}`, assignmentDoc);
  ...
  const deleteAssignments = useCallback(async () => {
    ...
    try {
      await deleteDoc(`carpoolAssignments/${selectedWeek}`);
